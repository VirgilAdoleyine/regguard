...
export class SidebarProvider implements vscode.WebviewViewProvider {
  static readonly viewType = 'regguard.sidebar';

  private view?: vscode.WebviewView;
  private allFindings: FileFindings[] = [];
  ...
  resolveWebviewView(
    ...
    webviewView.webview.onDidReceiveMessage(msg => {
      switch (msg.command) {
        case 'scan':         vscode.commands.executeCommand('regguard.scan');         break;
        case 'clearAll':     vscode.commands.executeCommand('regguard.clearFlags');   break;
        case 'openSettings': vscode.commands.executeCommand('workbench.action.openSettings', 'regguard'); break;
        case 'refreshRegs':  vscode.commands.executeCommand('regguard.refreshRegs'); break;
        case 'jumpToLine':   this.jumpToLine(msg.uri, msg.line);                      break;
        case 'applyFix':     vscode.commands.executeCommand('regguard.applyFixById', msg.uri, msg.findingId); break;
      }
    });
  }

  setScanning(scanning: boolean, msg = ''): void {
    this.isScanning = scanning;
    this.statusMsg  = msg;
    this.refresh();
  }

  updateFindings(findings: FileFindings[]): void {
  ...
  private async jumpToLine(uriStr: string, line: number): Promise<void> {
    try {
