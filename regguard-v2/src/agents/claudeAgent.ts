...
const parser = new JsonOutputParser<CodeFinding[]>();
...
export async function analyzeCode(
  ...
  trendingRegs:  Regulation[],
  openRouterKey: string
): Promise<CodeFinding[]> {
  const allRegs = [...regulations, ...trendingRegs];
  if (allRegs.length === 0 || !code.trim()) { return []; }
  ...
  const regSummary = allRegs.map(r =>
    `- [${r.id}] ${r.name} (${r.region}): ${r.description}` +
    (r.trending ? `\n  TRENDING: ${r.trendReason ?? ''}` : '')
  ).join('\n');

  const model  = buildModel('claude', openRouterKey, { temperature: 0.1, maxTokens: 4096 });
